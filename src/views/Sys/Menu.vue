  ...
  <div class="page-container">
	...
	<div class="toolbar" style="float:left;padding-top:10px;padding-left:15px;">
		<el-form :inline="true" :model="filters" :size="size">
			...
			<el-form-item>
				<kt-button icon="fa fa-search" :label="$t('action.search')" permissions="sys:menu:view" type="primary" @click="findTreeData(null)"/>
			</el-form-item>
			<el-form-item>
				<kt-button icon="fa fa-plus" :label="$t('action.add')" permissions="sys:menu:add" type="primary" @click="handleAdd"/>
			</el-form-item>
		</el-form>
	</div>
	<!--表格树内容栏-->
    <el-table :data="tableTreeDdata" stripe size="mini" style="width: 100%;"
      ...
      <el-table-column
        prop="permissions" header-align="center" align="center" width="150" 
        :show-overflow-tooltip="true" label="授权标识">
      </el-table-column>
      <el-table-column
        prop="orders" header-align="center" align="center" label="排序">
      </el-table-column>
      <el-table-column
        fixed="right" header-align="center" align="center" width="185" :label="$t('action.operation')">
        <template slot-scope="scope">
          <kt-button icon="fa fa-edit" :label="$t('action.edit')" permissions="sys:menu:edit" @click="handleEdit(scope.row)"/>
          <kt-button icon="fa fa-trash" :label="$t('action.delete')" permissions="sys:menu:delete" type="danger" @click="handleDelete(scope.row)"/>
        </template>
      </el-table-column>
    </el-table>
    <!-- 新增修改界面 -->
    <el-dialog :title="!dataForm.id ? '新增' : '修改'" width="40%" :visible.sync="dialogVisible" :close-on-click-modal="false">
      <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="submitForm()" 
        ...
        <el-form-item v-if="dataForm.type !== 0" label="授权标识" prop="permissions">
          <el-input v-model="dataForm.permissions" placeholder="如: sys:user:add, sys:user:edit, sys:user:delete"></el-input>
        </el-form-item>
        ...
        <el-form-item v-if="dataForm.type !== 2" label="排序编号" prop="orders">
          <el-input-number v-model="dataForm.orders" controls-position="right" :min="0" label="排序编号"></el-input-number>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    findTreeData: function() {
      this.loading = true;
      this.$api.menu.findMenuTree({'menuType': 2}).then(res => {
